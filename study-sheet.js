...
function revealSheetQuestion(){
 const root=[$('ai-bank-root'),$('hot100-root'),$('basic-root')].find(el=>!el.hidden),body=root.querySelector('.q-body');
...
function syncQuestionChat(){
 for(const [id,buttonId] of [['question','question-ask-ai'],['h-question','hot-ask-ai'],['b-question','basic-ask-ai']]){
...
for(const id of ['question','h-question','b-question'])new MutationObserver(syncQuestionChat).observe($(id),{childList:true,subtree:true});
for(const id of ['ai-bank-root','hot100-root','basic-root'])new MutationObserver(syncQuestionChat).observe($(id),{attributes:true,attributeFilter:['hidden']});
syncQuestionChat();
