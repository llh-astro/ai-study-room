...
function currentAssessmentSummary(){const data=bankData(),answers={},modules=[...data.ai.modules.map(x=>'岗位笔试 · '+x),...data.basics.modules.map(x=>'基础认知 · '+x),...data.enterprise.modules.map(x=>'企业决策 · '+x)];const questions=[];for(const [kind,key,offset] of [['ai',OLD,0],['basics',BASE,data.ai.modules.length],['enterprise',ENT,data.ai.modules.length+data.basics.modules.length]]){const prefix=kind==='ai'?'ai:':kind==='basics'?'basic:':'enterprise:';for(const q of data[kind].questions)questions.push({...q,id:prefix+q.id,module:q.module+offset});for(const [id,a] of Object.entries(parse(key,{}).answers||{}))answers[prefix+id]=a;}return assessmentSummary({ai:{modules,questions},hot:data.hot},answers,parse(HOT,{}));}
