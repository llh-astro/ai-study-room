...
(async()=>{
 ...
 const summary=JSON.parse(request.messages[1].content.split('\n').slice(1).join('\n'));assert.equal(summary.total.total,760);assert.equal(summary.total.answered,4);assert.equal(summary.modules.filter(m=>m.name.startsWith('企业决策')).length,7);assert.ok(summary.wrongExamples.some(q=>q.id==='enterprise:3'));assert.ok(!JSON.stringify(request).includes('private draft'));
