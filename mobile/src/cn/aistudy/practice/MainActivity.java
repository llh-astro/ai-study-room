...
public class MainActivity extends Activity {
 ...
 private static final Set<String> KEYS=new HashSet<>(Arrays.asList("ai-practice-150-v1","leetcode-hot100-python-v1","study-personal-v1","study-settings-v1","study-bank-v1"));
  ...
  web.setWebViewClient(new WebViewClient(){
   @Override public WebResourceResponse shouldInterceptRequest(WebView v,WebResourceRequest req){
    ...
    if("https".equals(uri.getScheme())&&"study.local".equals(uri.getHost())&&(uri.getPath().equals("/")||uri.getPath().equals("/index.html"))){
     try{Map<String,String> headers=new HashMap<>();headers.put("Content-Security-Policy","default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src data:; connect-src 'none'; frame-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'");return new WebResourceResponse("text/html","UTF-8",200,"OK",headers,getAssets().open("index.html"));}catch(Exception e){return blocked();}
    }
    return blocked();
   }
   @Override public boolean shouldOverrideUrlLoading(WebView v,WebResourceRequest req){
    Uri u=req.getUrl();if(ORIGIN.equals(u.toString())||(ORIGIN+"index.html").equals(u.toString()))return false;
    if(req.isForMainFrame()&&"https".equals(u.getScheme())&&"leetcode.cn".equals(u.getHost()))try{startActivity(new Intent(Intent.ACTION_VIEW,u));}catch(Exception ignored){}
    return true;
   }
